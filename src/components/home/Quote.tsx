  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full min-h-[80vh] bg-brand-secondary rounded-[32px] flex flex-col lg:flex-row items-center justify-center p-6 md:p-8 lg:p-12 shadow-2xl gap-10 lg:gap-20"
      >
        ...
        <div className="w-full lg:w-1/2 flex flex-col text-left text-white pt-8 lg:pt-0 lg:h-[75vh]">
          ...
          <div className="flex gap-1.5 mb-5 lg:mb-6">
            {[...Array(5)].map((_, i) => (
              <HiStar
                key={i}
                className="text-[20px] lg:text-[22px] text-white"
              />
            ))}
          </div>
          ...
          <h2 className="text-2xl md:text-3xl lg:text-[36px] font-medium leading-[1.3] tracking-tight mb-6 lg:mb-8">
            "Super clean clinic and a very friendly staff. The dentist took time
            to listen, offered options, and made sure I was comfortable the
            whole visit."
          </h2>
          ...
          <div className="flex flex-col gap-2 lg:gap-3 mb-6">
            ...
            <p className="text-white/90 text-[16px] md:text-[17px] leading-relaxed font-light max-w-[95%]">
              "They didn't rush anything. Every option was explained clearly,
          ...
          <div className="flex flex-wrap gap-2 lg:gap-3 mt-auto">
            {["Dental implants", "Crowns", "Whitening"].map((tag, i) => (
              <span
                key={i}
                className="bg-white/10 hover:bg-white/20 transition-colors cursor-pointer px-5 py-2.5 rounded-full text-sm font-medium text-white border border-white/5"
              >
                {tag}
              </span>
            ))}
